...
@v1.route("/suggestions/<int:suggestion_id>/dismiss", methods=["POST"])
@require_auth
def v1_dismiss_suggestion(suggestion_id: int):
    logger = logging.getLogger("API.V1")
    request_id = _get_request_id()
    user_id, error = _get_user_id_or_error()
    if error:
    ...
    results = _apply_suggestion_decisions(
        user_id,
        [{"suggestion_id": suggestion_id, "action": "reject", "reason": reason}],
        event_source="v1_dismiss",
    )
    logger.info(
        "API: v1 suggestion dismiss request_id=%s route=%s status=%s user_id=%s suggestion_id=%s",
        request_id,
        request.path,
        200,
        user_id,
    ...
    with get_connection() as conn:
        with conn.cursor() as cursor:
            ...
            if "goals" in normalized_scopes:
                ...
                # Clear pending goal drafts too
                cursor.execute("DELETE FROM suggestions WHERE user_id = %s AND kind = 'goal'", (user_id,))
                if "suggestions" in deleted:
